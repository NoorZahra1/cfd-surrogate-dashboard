...
def render_comparison_grid(case_name, params, param_label, resolution, lic_length):
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(5.8 * ncols, 4.8 * nrows), facecolor=BG)
...
@st.cache_data(show_spinner=False)
def run_optimization_sweep(case_name: str, objective_key: str, n_points: int):
    """
    Dense sweep of the parameter range, evaluating a scalar objective at
    each point using the surrogate (cheap: ~15-20ms/point even on the
    largest mesh). Returns the full curve so the whole design space trend
    is visible, not just a single optimum — more transparent and more
    useful for engineering judgment than a black-box optimizer result.
    """
    trainer = get_model(case_name)
    xy = trainer.pod.xy
    pmin, pmax = PARAM_RANGES[case_name]
    params = np.linspace(pmin, pmax, n_points)
    values = np.empty(n_points)
    for i, p in enumerate(params):
        result = predict_and_reconstruct(trainer, float(p))
        values[i] = compute_objective(objective_key, result, xy)
...
PARAM_RANGES = {
    "Cavity": (100, 10000),
    "Cylinder": (100, 10000),
    "Backward Facing Step": (100, 10000),
    "NACA0012": (-5.0, 15.0),
}
...
tab_overview, tab_flow, tab_compare, tab_optimize, tab_explorer, tab_diag, tab_validate = st.tabs(
    ["Overview", "Flow Visualization", "Parameter Comparison", "Design Optimization", "Field Explorer", "Model Diagnostics", "Validate vs Ground Truth"]
)
...
# ----------------------------------------------------------------------
# TAB — Design Optimization
# ----------------------------------------------------------------------
with tab_optimize:
    st.caption(
        f"Sweep the full {param_label} range and find the value that minimizes or maximizes "
        "an engineering objective — using the surrogate as a fast stand-in for running full CFD "
        "at every candidate value."
    )
 
    available_objectives = {
        k: v for k, v in OBJECTIVE_META.items() if v["cases"] is None or case in v["cases"]
    }
    obj_key = st.selectbox(
        "Objective", list(available_objectives.keys()),
        format_func=lambda k: available_objectives[k]["label"],
    )
    if obj_key == "pressure_asymmetry":
        st.caption(
            "Note: this is an illustrative proxy (upper-region mean pressure minus lower-region "
            "mean pressure), not a validated lift coefficient — the mesh here isn't tagged with "
            "airfoil-surface nodes, which a rigorous Cl calculation would require."
        )
 
    col_a, col_b, col_c = st.columns(3)
    direction = col_a.radio("Direction", ["Minimize", "Maximize"], horizontal=True)
    n_points = col_b.slider("Sweep resolution", 50, 400, 200, 25)
    run_btn = col_c.button("Run Optimization Sweep", use_container_width=True, type="primary")
 
    opt_key = f"opt_{case}_{obj_key}_{n_points}"
    if run_btn:
        st.session_state[opt_key] = True
 
    if st.session_state.get(opt_key):
        t0 = time.time()
        with st.spinner(f"Evaluating {n_points} candidate {param_label} values..."):
            params, values = run_optimization_sweep(case, obj_key, n_points)
        elapsed = time.time() - t0
 
        valid = ~np.isnan(values)
        if not valid.any():
            st.error("All objective values were NaN — this objective may not be well-defined for this case.")
        else:
            params_v, values_v = params[valid], values[valid]
            best_idx = int(np.argmin(values_v)) if direction == "Minimize" else int(np.argmax(values_v))
            best_param, best_value = float(params_v[best_idx]), float(values_v[best_idx])
 
            m1, m2, m3 = st.columns(3)
            m1.metric(f"Optimal {param_label}", f"{best_param:g}")
            m2.metric(available_objectives[obj_key]["label"], f"{best_value:.4g}")
            m3.metric("Sweep time", f"{elapsed:.2f}s", help=f"{n_points} surrogate evaluations")
 
            fig_sweep = go.Figure()
            fig_sweep.add_trace(go.Scatter(
                x=params_v, y=values_v, mode="lines", line=dict(color=ACCENT, width=2), name="Objective"
            ))
            fig_sweep.add_trace(go.Scatter(
                x=[best_param], y=[best_value], mode="markers",
                marker=dict(color="#f97316", size=14, symbol="star"), name="Optimum"
            ))
            fig_sweep.update_layout(
                title=f"{available_objectives[obj_key]['label']} vs. {param_label}",
                xaxis_title=param_label, yaxis_title=available_objectives[obj_key]["label"],
                height=420, template="plotly_dark",
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            )
            st.plotly_chart(fig_sweep, use_container_width=True)
 
            st.markdown(f"**Flow field at the optimum ({param_label} = {best_param:g}):**")
            blended_opt, gx_opt, gy_opt, smin_opt, smax_opt = compute_lic_image(case, best_param, resolution, lic_length)
            fig_opt, ax_opt = plt.subplots(figsize=(7, 5.5), facecolor=BG)
            ax_opt.set_facecolor(BG)
            ax_opt.imshow(blended_opt, origin="lower", extent=[gx_opt.min(), gx_opt.max(), gy_opt.min(), gy_opt.max()], aspect="equal")
            ax_opt.set_xticks([]); ax_opt.set_yticks([])
            ...
            st.download_button(
                "Download sweep data as CSV",
                data=pd.DataFrame({param_label: params_v, available_objectives[obj_key]["label"]: values_v}).to_csv(index=False).encode("utf-8"),
                file_name=f"{case.replace(' ', '_').lower()}_{obj_key}_sweep.csv",
                mime="text/csv",
            )
    else:
        st.info("Choose an objective and click **Run Optimization Sweep**.")
