...
OBJECTIVE_META = {
    "mean_speed": {"label": "Mean velocity magnitude", "cases": None},
    "max_speed": {"label": "Max velocity magnitude", "cases": None},
    "pressure_range": {"label": "Pressure range (max \u2212 min)", "cases": None},
    "mean_pressure": {"label": "Mean pressure", "cases": None},
    "pressure_asymmetry": {
        "label": "Pressure asymmetry (illustrative lift-like proxy, not a rigorous Cl)",
        "cases": ["NACA0012"],
    },
    "reattachment_length": {
        "label": "Reattachment length (recirculation zone extent)",
        "cases": ["Backward Facing Step"],
    },
}


def compute_objective(objective_key: str, result: Dict[str, np.ndarray], xy: np.ndarray) -> float:
    """
    Scalar engineering objective computed from a predicted field, used to
    drive parameter-sweep optimization. These are simple, transparent
    quantities chosen to be meaningful without requiring surface-specific
    mesh tagging (e.g. true lift/drag coefficients would need identified
    airfoil-surface nodes and a proper surface integral, which this mesh
    representation doesn't carry) — `pressure_asymmetry` is explicitly
    labeled as an illustrative proxy, not a validated aerodynamic
    coefficient, and should not be reported as one.
    """
    u, v, p = result["u"], result["v"], result["p"]
    if objective_key == "mean_speed":
        return float(np.mean(np.sqrt(u ** 2 + v ** 2)))
    if objective_key == "max_speed":
        return float(np.max(np.sqrt(u ** 2 + v ** 2)))
    if objective_key == "pressure_range":
        return float(np.max(p) - np.min(p))
    if objective_key == "mean_pressure":
        return float(np.mean(p))
    if objective_key == "pressure_asymmetry":
        upper = p[xy[:, 1] > 0]
        lower = p[xy[:, 1] <= 0]
        if len(upper) == 0 or len(lower) == 0:
            return float("nan")
        return float(np.mean(upper) - np.mean(lower))
    if objective_key == "reattachment_length":
        return _compute_reattachment_length(u, xy)
    raise ValueError(f"Unknown objective: {objective_key}")


def _compute_reattachment_length(u: np.ndarray, xy: np.ndarray) -> float:
    """
    Classic Backward Facing Step metric: the distance downstream of the
    step where the recirculating flow (negative streamwise velocity along
    the bottom wall) reattaches (u turns positive again). Found by
    selecting nodes near the bottom wall, sorting by x, and locating the
    last sign change from negative to positive.
    """
    y_min = xy[:, 1].min()
    tol = (xy[:, 1].max() - y_min) * 0.02
    near_wall = np.abs(xy[:, 1] - y_min) < tol
    if near_wall.sum() < 10:
        return float("nan")

    xs, us = xy[near_wall, 0], u[near_wall]
    order = np.argsort(xs)
    xs_sorted, us_sorted = xs[order], us[order]

    # bin by x (multiple nodes can share similar x) and average u per bin
    # for a cleaner, less noisy sign-change detection
    n_bins = min(200, len(xs_sorted))
    bin_edges = np.linspace(xs_sorted.min(), xs_sorted.max(), n_bins + 1)
    bin_idx = np.digitize(xs_sorted, bin_edges) - 1
    bin_idx = np.clip(bin_idx, 0, n_bins - 1)
    bin_u = np.array([us_sorted[bin_idx == i].mean() if (bin_idx == i).any() else np.nan for i in range(n_bins)])
    bin_x = 0.5 * (bin_edges[:-1] + bin_edges[1:])

    valid = ~np.isnan(bin_u)
    bin_x, bin_u = bin_x[valid], bin_u[valid]
    if len(bin_u) < 3:
        return float("nan")

    # find the last index where u is negative, immediately followed by positive
    sign_changes = np.where((bin_u[:-1] < 0) & (bin_u[1:] >= 0))[0]
    if len(sign_changes) == 0:
        return float("nan")
    reattach_x = bin_x[sign_changes[-1] + 1]
    step_x = xy[:, 0].min()
    return float(reattach_x - step_x)
